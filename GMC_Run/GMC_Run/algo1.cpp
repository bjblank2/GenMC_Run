    ...
    for (int i = 0; i < chem_motif_groups[site].size(); i++) {
        ...
        for (int j = 0; j < motif.size(); j++) {
            ...
            for (int k = 0; k < group.size(); k++) {
                rule_key += "." + to_string(chem_list[j]); // sites
            }
...
float Algo1::eval_site_spin(int site) {
    ...
    for (int i = 0; i < spin_motif_groups[site].size(); i++) {
        ...
        for (int j = 0; j < motif.size(); j++) {
            ...
            enrg += (rule_itr != rule_map_spin.end()) ? (rule_itr->second / group.size()) : 0.0;
        }
    }
    return enrg;
}
    ...
    for (int i = 0; i < spin_motif_groups[site].size(); i++) {
        ...
        for (int j = 0; j < motif.size(); j++) {
            ...
            for (int k : group) {
                ...
                if (k != site) { spin_prod *= spin_list[j]; }
            }
            rule_itr = rule_map_spin.find(rule_key);
...
float Algo1::eval_atom_flip(int site) {
    ...
    for (int i = 0; i < spin_motif_groups[site].size(); i++) {
        ...
        for (int j = 0; j < motif.size(); j++) {
            ...
            rule_itr = rule_map_spin.find(rule_key);
            enrg += (rule_itr != rule_map_spin.end()) ? (rule_itr->second) : 0.0;
        }
    }
    return enrg;
}
...
void Algo1::print_state(float temp) {
    vector<int> perm;
